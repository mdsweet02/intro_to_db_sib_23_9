...
CREATE TABLE Log_Sportsmeny
(LogID INT IDENTITY PRIMARY KEY,
Operation VARCHAR(20),
SportsmenID INT,
FIO_Nazvanie VARCHAR(100),
DateTimeAction DATETIME DEFAULT GETDATE());


--1. Insert
CREATE TRIGGER trg_Sportsmeny_Insert
ON Sportsmeny
AFTER INSERT
AS
BEGIN
INSERT INTO Log_Sportsmeny(Operation, SportsmenID, FIO_Nazvanie)
SELECT 'INSERT', SportsmenID, FIO_Nazvanie
FROM inserted;
END;
GO

INSERT INTO Sportsmeny (SportsmenID, FIO_Nazvanie, SportID, TrenerID, Priznak) VALUES (21, 'Иван Иванов', 1, 1, 'Физ.Лицо');
SELECT * FROM Log_Sportsmeny;


--2.Update
CREATE TRIGGER trg_Sportsmeny_Update
ON Sportsmeny
AFTER UPDATE
AS
BEGIN
INSERT INTO Log_Sportsmeny(Operation, SportsmenID, FIO_Nazvanie)
SELECT 'UPDATE', SportsmenID, FIO_Nazvanie
FROM inserted;
END;
GO

UPDATE Sportsmeny
SET FIO_Nazvanie = 'Пётр Петров'
WHERE SportsmenID = 21;
SELECT * FROM Log_Sportsmeny;

--3.Delete
CREATE TRIGGER trg_Sportsmeny_Delete
ON Sportsmeny
AFTER DELETE
AS
BEGIN
INSERT INTO Log_Sportsmeny(Operation, SportsmenID, FIO_Nazvanie)
SELECT 'DELETE', SportsmenID, FIO_Nazvanie
FROM deleted;
END;
GO

DELETE FROM Sportsmeny
WHERE SportsmenID = 21;
SELECT * FROM Log_Sportsmeny;
