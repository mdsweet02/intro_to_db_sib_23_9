...
BEGIN
    ...
    PRINT 'Доступные экземпляры уменьшены';
END;
...
BEGIN
    ...
    PRINT 'Экземпляр возвращён на склад';
END;
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            PRINT 'Ошибка: изменение цены более чем на 20% запрещено!';
            ROLLBACK TRANSACTION;
        END
        ELSE
            PRINT 'Цена изменена корректно';
    END
END;
...
BEGIN
    ...
    PRINT 'В диск добавлено музыкальное произведение';
END;
